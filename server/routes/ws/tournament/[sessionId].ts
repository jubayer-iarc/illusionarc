// server/routes/ws/tournament/[sessionId].ts
...
function validateHelloAgainstTicket(
  ...
  const tournamentSlug = String(hello.tournamentSlug || '').trim()
  ...
  if (payload.slug !== tournamentSlug) {
    return { ok: false as const, reason: 'Ticket slug mismatch' }
  }
...
export default defineWebSocketHandler({
  ...
  async message(peer, message) {
    ...
    if (!state.authenticated) {
      ...
      state.tournamentSlug = result.tournamentSlug
      state.tournamentId = result.tournamentId
