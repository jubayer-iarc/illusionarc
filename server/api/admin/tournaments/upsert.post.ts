...
export default defineEventHandler(async (event) => {
  ...
  const finalPromoVideoUrl =
    finalPromoVideoType === 'upload' ? promo_video_url : null
  const finalPromoVideoPath =
    finalPromoVideoType === 'upload' ? promo_video_path : null
  const finalPromoVideoYoutubeId =
    finalPromoVideoType === 'youtube' ? promo_video_youtube_id : null
  const finalPromoVideoTitle =
    finalPromoVideoType ? promo_video_title : null

  {
    let check = db.from('tournaments').select('id').eq('slug', slug)
