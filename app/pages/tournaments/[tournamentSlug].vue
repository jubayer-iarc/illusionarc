...
/**
 * ✅ Do NOT set unknown middleware here.
 * This page is public (viewable). You gate the Play button only.
 */
definePageMeta({})
...
/**
 * ✅ Requirement:
 * Arcade button should be visible UNTIL tournament goes LIVE.
 * Once LIVE -> hide Arcade button.
 */
...
const statusBadge = computed(() => {
  ...
  if (s === 'live')
    return { text: 'LIVE', cls: 'bg-emerald-500/15 border border-emerald-400/25 text-emerald-200', dot: 'bg-emerald-400' }
  if (s === 'scheduled')
    return { text: 'SCHEDULED', cls: 'bg-violet-500/15 border border-violet-400/25 text-violet-200', dot: 'bg-violet-400' }
  if (s === 'canceled')
    return { text: 'CANCELED', cls: 'bg-red-500/15 border border-red-400/25 text-red-200', dot: 'bg-red-400' }
  return { text: 'ENDED', cls: 'bg-white/10 border border-white/12 text-white/75', dot: 'bg-white/60' }
})
...
<template>
  <UContainer class="py-10">
    ...
    <div v-else>
      ...
      <div class="mt-5 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
        ...
        <div class="group relative overflow-hidden rounded-3xl border border-white/10 bg-white/5">
          ...
          <div class="absolute left-4 top-4 flex flex-wrap items-center gap-2">
            <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold" :class="statusBadge.cls">
            ...
            <span v-if="sub?.active" class="inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-500/10 px-3 py-1 text-[11px] text-emerald-100/90">
          ...
          <div class="relative p-6 sm:p-7">
            ...
            <div class="mt-4 flex flex-wrap items-center gap-2 text-sm text-white/85">
              <span class="inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/35 px-3 py-1.5">
        ...
        <div class="space-y-4">
          <div class="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div class="flex items-start justify-between gap-3">
              <div>
                ...
                <div class="mt-1 text-xs opacity-70">Live is computed by time window.</div>
              </div>
              <span class="text-[11px] opacity-70 font-mono">{{ slug }}</span>
            </div>
            ...
            <div class="mt-4 grid gap-3">
              <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div v-if="isScheduled" class="mt-2 text-sm opacity-80">
              ...
              <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div v-if="isLive" class="mt-2 text-sm opacity-80">
          ...
          <div class="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm">
            <div class="flex items-center justify-between gap-3">
              <div class="text-sm font-semibold">Eligibility</div>
              <span class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-3 py-1 text-[11px] opacity-90">
                <span class="inline-flex h-1.5 w-1.5 rounded-full" :class="sub?.active ? 'bg-emerald-400' : 'bg-white/35'" />
          ...
          <div class="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div class="flex items-center justify-between gap-3">
              <div>
                ...
                <div class="mt-1 text-xs opacity-70">Top 3 winners will be rewarded.</div>
              </div>
              <UIcon name="i-heroicons-gift" class="h-5 w-5 opacity-80" />
            </div>

            <div class="mt-4 grid gap-3">
              <div v-for="p in prizeTriplet" :key="p.rank" class="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-xs opacity-70">{{ p.label }}</div>
                  <div class="text-base">{{ p.rank === 1 ? '🥇' : p.rank === 2 ? '🥈' : '🥉' }}</div>
                </div>
                <div class="mt-1 text-sm font-semibold">{{ p.value }}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Winners (ENDED only) -->
      <div v-if="isEnded" class="mt-8 rounded-3xl border border-white/10 bg-white/5 p-6 overflow-hidden">
        <div class="flex items-center justify-between gap-3">
          <div>
        ...
        <div v-else class="mt-6">
          ...
          <div class="mt-6 grid gap-3 md:grid-cols-3">
            <div v-for="r in winners" :key="`${r.rank}-${r.player_name}-${r.score}`" class="rounded-2xl border border-white/10 bg-white/5 p-4">
              <div class="flex items-center justify-between gap-3">
        ...
        <div class="lg:col-span-2 space-y-4">
          <div class="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div class="flex items-center justify-between gap-3">
              ...
              <UIcon name="i-heroicons-information-circle" class="h-6 w-6 opacity-75" />
            </div>

            <div class="mt-4 grid gap-3 sm:grid-cols-2">
              <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div class="text-xs opacity-70">Entry</div>
              ...
              <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div class="mt-1 text-sm font-semibold">{{ game?.name || getGameSlug(t) }}</div>
                <div class="mt-2 text-xs opacity-70">
                  Practice before live. When live, play from tournament page.
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-3xl border border-white/10 bg-white/5 p-5">
            <div class="flex items-center justify-between gap-3">
              ...
              <UIcon name="i-heroicons-sparkles" class="h-6 w-6 opacity-75" />
            </div>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
              ...
              <UButton :to="`/tournaments/embed/${t.slug}`" variant="soft" class="!rounded-full">
                <UIcon name="i-heroicons-arrow-top-right-on-square" class="h-5 w-5" />
                Open embed
              </UButton>
          ...
          <div v-else class="mt-4 space-y-2">
            ...
            >
              <div class="flex items-center gap-2 min-w-0">
                ...
                <div class="text-sm font-medium truncate">{{ safeName(r.player_name) }}</div>
              </div>
              <div class="text-sm font-semibold">{{ r.score }}</div>
            </div>

            <div v-if="lbPending" class="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm opacity-70">
              Loading…
            </div>
          </div>
        </div>
