...
function rankChipClass(rank: number) {
  if (rank === 1) return 'border-amber-300/40 bg-amber-400/15 text-amber-700 dark:text-amber-200'
  if (rank === 2) return 'border-slate-300/40 bg-slate-300/15 text-slate-700 dark:text-slate-200'
  if (rank === 3) return 'border-orange-300/40 bg-orange-400/15 text-orange-700 dark:text-orange-200'
...
function leaderboardRowClass(rank: number) {
  if (rank === 1) return 'leader-row-top leader-row-gold'
  if (rank === 2) return 'leader-row-top leader-row-silver'
  if (rank === 3) return 'leader-row-top leader-row-bronze'
  return ''
}
function leaderboardRankTextClass(rank: number) {
  if (rank <= 3) return 'text-slate-950 dark:text-white'
  return 'text-slate-900 dark:text-white'
}
function leaderboardNameClass(rank: number) {
  if (rank <= 3) return 'font-extrabold tracking-tight'
  return 'font-semibold'
}
function leaderboardScoreClass(rank: number) {
...
const showPlayButtons = computed(() => !isEnded.value && !isCanceled.value)
...
const heroDescription = computed(() => {
  return (
      cleanText(t.value?.description) ||
      'এই টুর্নামেন্টের বিস্তারিত, নিয়ম, সময়সূচী এবং পুরস্কারের তথ্য নিচে দেওয়া আছে।'
  )
})

const disclaimerTitle = '⚠️ DISCLAIMER — অবশ্যই পড়ুন'
const disclaimerIntro = 'ওয়েবসাইটে দেওয়া সকল Disclaimer মেনে চলা বাধ্যতামূলক।'

const disclaimerList = computed(() => {
  return [
    'আমাদের ইনবিল্ট Floating Recorder ব্যবহার করুন। কোনো সমস্যা হলে আপনার ডিভাইসের নিজস্ব রেকর্ডার ব্যবহার করতে পারবেন।',
    'সর্বোচ্চ স্কোর করা সেশনের সম্পূর্ণ ভিডিওটি জমা দিন।',
    'শুধু Highest Score সেশনের রেকর্ডিং জমা দিলেই হবে — বাকি সব রেকর্ডিং ডিভাইস থেকে মুছে ফেলতে পারেন।',
    'কাটা-ছেঁড়া, আংশিক, ক্লিপ করা বা জোড়া দেওয়া কোনো ভিডিও গ্রহণযোগ্য হবে না।'
  ]
})

const termsList = computed(() => {
  return [
    'গেমটি লাইভ থাকা অবস্থায় খেললেই স্কোর টুর্নামেন্টে গণনা হবে।',
    'অংশ নিতে ও স্কোর জমা দিতে লগ-ইন এবং সক্রিয় সাবস্ক্রিপশন বাধ্যতামূলক।',
    ...
    'গেমপ্লে রেকর্ডিং সংক্রান্ত Disclaimer এবং Recorder ব্যবহারের নির্দেশনা মেনে চলা বাধ্যতামূলক।',
...
const visibleWinners = computed(() => winners.value.slice(0, 20))
...
const visiblePrizes = computed(() => {
  ...
  for (const w of winners.value || []) {
    ...
    if (!p) continue
    const rank = Number((p as any).rank || w.rank || 0)
          ...
          <div class="min-w-0 space-y-5 sm:space-y-6">
            ...
            >
              ...
              <div class="relative space-y-4">
                ...
                <div class="max-w-4xl whitespace-pre-line text-sm leading-7 text-black/70 sm:text-base dark:text-white/75">
                  {{ heroDescription }}
                </div>

                <div class="disclaimer-box rounded-[22px] border p-4 sm:p-5">
                  <div class="disclaimer-head flex items-start gap-3">
                    <div class="mt-0.5 text-xl sm:text-2xl">⚠️</div>
                    <div class="min-w-0">
                      <div class="text-sm font-extrabold tracking-[0.12em] text-white/90 sm:text-base">
                        DISCLAIMER — অবশ্যই পড়ুন
                      </div>
                      <div class="mt-2 text-sm leading-7 text-white/90 sm:text-[15px]">
                        {{ disclaimerIntro }}
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="mt-4">
                    <div class="text-sm font-bold text-white sm:text-[15px]">
                      গেমপ্লে রেকর্ডিং সংক্রান্ত নির্দেশাবলি:
                    </div>

                    <ul class="mt-3 space-y-3">
                      <li
                          v-for="(item, i) in disclaimerList"
                          :key="`disclaimer-${i}`"
                          class="flex gap-3"
                      >
                        <span class="disclaimer-num mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-extrabold">
                          {{ toBnDigits(i + 1) }}
                        </span>
                        <span class="text-sm leading-7 text-white/95 sm:text-[15px]">
                          {{ item }}
                        </span>
                      </li>
                    </ul>
            ...
            <div class="xl:hidden">
              ...
              >
                <div class="rounded-[25px] bg-white p-4 sm:rounded-[29px] sm:p-6 dark:bg-[#0b1322]">
                  ...
                  <div v-else class="mt-6 rounded-[18px] border border-red-400/20 bg-red-500/10 px-4 py-4 text-center text-sm font-semibold text-red-700 dark:text-red-200 sm:mt-8">
                    {{ isCanceled ? 'এই টুর্নামেন্ট বাতিল করা হয়েছে' : 'টুর্নামেন্ট শেষ হয়েছে' }}
                  </div>
              ...
              <div v-else class="mt-4 space-y-3">
                ...
                >
                  ...
                  <div class="flex min-w-0 items-center gap-3">
                    <div class="w-7 shrink-0 text-center text-xl">{{ medal(i + 1) }}</div>
                    <div class="min-w-0">
                      <div class="text-sm font-bold" :class="leaderboardRankTextClass(i + 1)">
                        {{ toBnDigits(i + 1) }}
                      </div>
                    </div>
                    <div
                    ...
                    >
                      <div class="flex min-w-0 items-center gap-3">
                        <div class="w-12 shrink-0 text-center text-base font-extrabold text-slate-900 dark:text-white">
                          {{ medal(w.rank) }} {{ toBnDigits(w.rank) }}
                        </div>
                      ...
                      <div class="shrink-0 text-right">
                        <div class="text-xs text-black/55 dark:text-white/55">স্কোর</div>
              ...
              <div class="rounded-[24px] border border-black/10 bg-white p-4 shadow-[0_14px_44px_rgba(15,23,42,0.08)] sm:rounded-[26px] sm:p-6 dark:border-white/10 dark:bg-white/5 dark:shadow-[0_18px_46px_rgba(0,0,0,0.30)]">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <div class="mt-2 text-sm leading-7 text-black/70 sm:text-base dark:text-white/75">
                      নতুন টুর্নামেন্ট, আপডেট, ঘোষণা এবং কনটেন্ট সবার আগে পেতে আমাদের সোশ্যাল মিডিয়ায় যুক্ত থাকুন।
                    </div>
                  </div>
...
.disclaimer-box {
  ...
  background:
      linear-gradient(135deg, rgba(153, 27, 27, 0.96), rgba(220, 38, 38, 0.95));
  box-shadow:
      0 0 0 1px rgba(248, 113, 113, 0.14),
      0 18px 36px rgba(220, 38, 38, 0.18);
}
...
.disclaimer-head,
.disclaimer-box ul,
.disclaimer-box .text-sm,
.disclaimer-box .text-base {
  position: relative;
  z-index: 1;
}
...
:global(.dark) .leader-row-silver {
  border-color: rgba(226, 232, 240, 0.18) !important;
  background: rgba(148, 163, 184, 0.12) !important;
}

:global(.dark) .leader-row-bronze {
